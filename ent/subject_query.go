...
func (sq *SubjectQuery) QueryCourses() *CourseQuery {
...
func (sq *SubjectQuery) WithCourses(opts ...func(*CourseQuery)) *SubjectQuery {
	query := (&CourseClient{config: sq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	sq.withCourses = query
	return sq
}
...
func (sq *SubjectQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Subject, error) {
	...
	if query := sq.withCourses; query != nil {
		if err := sq.loadCourses(ctx, query, nodes,
			func(n *Subject) { n.Edges.Courses = []*Course{} },
			func(n *Subject, e *Course) { n.Edges.Courses = append(n.Edges.Courses, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (sq *SubjectQuery) loadCourses(ctx context.Context, query *CourseQuery, nodes []*Subject, init func(*Subject), assign func(*Subject, *Course)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(subject.CoursesTable)
		s.Join(joinT).On(s.C(course.FieldID), joinT.C(subject.CoursesPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(subject.CoursesPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(subject.CoursesPrimaryKey[0]))
